...
QDAP delivers \textbf{74.0\%} of DENMs within the 50~ms deadline at $N{=}100$
versus \textbf{49.2\%} for DSRC (+50\%, $p < 0.0001$) and \textbf{31.1\%}
for C-V2X Mode~4 (2.4$\times$, $p{=}0.003$), with emergency p99 latency of
\SI{2.8}{\milli\second} invariant to vehicle density.
An ablation study confirms that adaptive FEC is the primary PDR driver
($-6.2$~pp without it) while the QFT scheduler provides the latency guarantee
(3.4$\times$ reduction).
The implementation ships with a Rust-accelerated core (\texttt{qdap\_core},
PyO3/maturin, $\geq$2.6~M FEC decisions/s) and a pure-Python fallback for
ECUs without a Rust toolchain; both paths are verified against identical test
vectors.
The stack is open-source (MIT) and available via \texttt{pip install qdap}.
\end{abstract}
...
A subtler risk is that C-V2X's high delivery PDR (64--67\%) creates
a false sense of reliability: the system \emph{does} deliver most DENMs,
just not within the safety window.
A V2X deployment engineer observing PDR metrics without checking deadline
compliance would conclude that C-V2X outperforms DSRC for emergency
messages — the opposite of the safety-relevant truth.

% ── Contributions ────────────────────────────────────────────────────────────
The main contributions of this work are:

\begin{enumerate}
  \item \textbf{QDAP protocol design}: A dual-backend application-layer
        protocol with four tightly integrated components: a QFT-inspired
        scheduler, adaptive FEC, 0-RTT Ghost Session, and delta encoder.
        A Rust core (\texttt{qdap\_core}, PyO3) provides $\geq$2.6~M FEC
        decisions/s for embedded ECUs; a pure-Python fallback ensures
        compatibility without a Rust toolchain.
        Both paths are verified against identical test vectors.
        The protocol runs over existing TCP/UDP infrastructure
  ...
  \item \textbf{Drop-in MAC adapters}: Bridge layer implementations for
        QDAP over DSRC (EDCA MAC) and QDAP over C-V2X (SPS + aperiodic
        emergency TX), showing $+21$~pp gain over bare DSRC and $+47$~pp
        over bare C-V2X deadline compliance — without changing any radio
        hardware or MAC parameters.
...
DSRC uses the empirical broadcast collision model ($p_{\text{col}} = \min(\mathrm{CBR}^2
\times 0.6,\; 0.55)$)~\cite{Sepulcre2011}, applied on top of the link-layer PER.
C-V2X SPS delay is modeled as $\mathcal{U}[0, 100]$~ms per
3GPP~TS~36.213~\cite{3GPP_TS_36213}.
UDP is included as a transport-layer baseline without reliability mechanisms.
MQTT is included as a \emph{V2I broker-mediated} baseline — representative of
infrastructure-relayed safety alert pipelines proposed in the
literature~\cite{Lyu2021} — and is not a direct V2V protocol; its model
includes local RSU broker round-trip, TCP retransmit timeout (\SI{200}{\milli\second}),
and HOL blocking proportional to load.

QDAP is modeled as an application-layer protocol operating over the same
physical channel as DSRC (same SNR $\to$ PER mapping).
MAC-layer collision probability is applied using the same broadcast model,
with a priority-access reduction factor derived from the IEEE~802.11e EDCA
AC\_VO / AC\_BE contention window ratio ($\approx\!0.25\times$ for emergency,
$0.6\times$ for VRU, $1.0\times$ for normal traffic).
The effective per-packet loss rate combines channel PER and collision:
$\tilde{p} = 1 - (1-p_{\text{ch}})(1-p_{\text{col}})$, and FEC operates
on $\tilde{p}$ (Eq.~\ref{eq:fec}).
FEC profile selection and exact binomial failure probability use the
\texttt{qdap\_core} Rust library.
A sensitivity study confirmed that emergency DENM PDR is invariant to the
collision model ($\pm 0.1$~pp) because the emergency FEC profile
($k{=}1, r{=}2$) absorbs the added loss: at CBR~$=\!0.36$,
$p_{\text{col,emerg}} = 1.95\%$, increasing $\tilde{p}$ by $<\!2$~pp,
which the three-redundancy code handles with $<\!0.01\%$ marginal failure
probability.
...
\begin{table}[htbp]
\centering
\caption{Urban Intersection Results ($N{=}75$, 20 MC runs, mean $\pm$ 95\% CI)}
\label{tab:urban}
\begin{tabular}{lrrrr}
\toprule
Protocol      & DENM PDR       & $<$50~ms       & BSM PDR        & Emerg.~p99 \\
              & (\%)           & (\%)           & (\%)           & (ms) \\
\midrule
\textbf{QDAP} & \textbf{74.2$\pm$5.6} & \textbf{74.2} & 62.8$\pm$1.0   & \textbf{2.8}\\
802.11bd      & 73.5$\pm$4.7   & 73.5           & \textbf{67.5$\pm$1.0}  & 1.5  \\
C-V2X Mode~4  & 66.9$\pm$4.8   & 32.5$\pm$2.4\textsuperscript{$\dagger$} & 61.0$\pm$1.0 & 100.3 \\
DSRC 802.11p  & 53.5$\pm$4.8\textsuperscript{$*$}   & 53.5           & 56.3$\pm$1.0   & 2.4  \\
UDP           & 66.0$\pm$5.8\textsuperscript{$*$}   & 66.0           & 58.9$\pm$1.1   & 9.2  \\
MQTT          & 59.4$\pm$6.3\textsuperscript{$*$}   & 48.6$\pm$5.4   & 53.1$\pm$1.1   & 240.6 \\
\bottomrule
\multicolumn{5}{l}{\footnotesize $\dagger$ SPS structural gap: 34.4~pp between DENM PDR and deadline compliance.} \\
\multicolumn{5}{l}{\footnotesize $*$ $p < 0.05$ vs QDAP, Mann-Whitney U (one-sided).}
\end{tabular}
\end{table}
...
$N{=}100$.
QDAP's advantage over DSRC is statistically significant at all tested
densities ($p < 0.0001$ at $N{=}75$, Mann-Whitney U, one-sided), and
over C-V2X at $N{=}75$ ($p = 0.016$, $*$) and $N{=}100$ ($p = 0.003$, $**$).
All real urban intersections operate at CBR $> 0.30$ above $N \approx 60$,
placing them in the regime where QDAP holds a persistent, significant advantage.
...
\begin{table}[htbp]
\centering
\caption{QDAP Ablation Study — Urban $N{=}75$ (20 MC runs, mean $\pm$ 95\% CI)}
\label{tab:ablation}
\begin{tabular}{lrrrr}
\toprule
Variant          & DENM PDR       & $<$50~ms & Emerg.~p99 & $\Delta$ PDR \\
                 & (\%)           & (\%)     & (ms)       & (pp)\\
\midrule
QDAP (full)      & \textbf{74.2$\pm$5.6} & \textbf{74.2} & \textbf{2.8}  & —      \\
w/o FEC          & 68.0$\pm$6.0   & 68.0     & 2.9        & $-6.2$ \\
w/o Scheduler    & 74.2$\pm$5.6   & 74.2     & 9.5        & 0.0    \\
w/o Ghost Sess.  & 74.3$\pm$5.4   & 74.2     & 24.9       & $+0.1$ \\
w/o Delta Enc.   & 74.2$\pm$5.6   & 74.2     & 2.8        & 0.0    \\
\bottomrule
\end{tabular}
\end{table}
...
removing it drops PDR by 6.2~pp (74.2\%~$\to$~68.0\%) with no latency impact,
...
(\SI{9.5}{\milli\second}~$\to$~\SI{2.8}{\milli\second}) by preempting
BSM queue drain with immediate DENM transmission.
The \textit{Ghost Session} has negligible PDR impact ($<0.1$~pp) but reduces
tail latency by $8.9\times$ (\SI{24.9}{\milli\second}~$\to$~\SI{2.8}{\milli\second}),
...
\subsection{QDAP as a Drop-In Application Layer}
\label{sec:dropin}

The comparisons above treat each protocol as a complete, independent stack.
A practitioner question is different: \textit{given existing V2X radio hardware,
what does adding QDAP's application layer contribute?}
To answer this, we evaluate two combined stacks:
\textbf{QDAP+DSRC} (QDAP application layer over IEEE~802.11p EDCA MAC) and
\textbf{QDAP+C-V2X} (QDAP application layer over C-V2X Mode~4 sidelink with
aperiodic one-shot TX for emergency messages, as permitted by
3GPP~TS~36.321~\S{}5.8.1~\cite{3GPP_TS_36213}).

Table~\ref{tab:dropin} shows results at $N{=}75$ (urban intersection).

\begin{table}[htbp]
\centering
\caption{QDAP Drop-In Gain Over Bare MAC ($N{=}75$, 5 MC runs)}
\label{tab:dropin}
\begin{tabular}{lrrrl}
\toprule
Stack              & DENM PDR & $<$50~ms & Emerg.~p99 & Gain \\
                   & (\%)     & (\%)     & (ms)       & (pp) \\
\midrule
QDAP (standalone)  & 74.2     & 74.2     & 2.9        & ref  \\
\midrule
DSRC 802.11p       & 53.8     & 53.8     & 2.5        & —    \\
\textbf{QDAP+DSRC} & \textbf{75.1} & \textbf{75.1} & \textbf{1.4} & \textbf{+21.3}\\
\midrule
C-V2X Mode~4       & 67.2     & 33.2\textsuperscript{$\dagger$} & 100.5 & — \\
\textbf{QDAP+C-V2X}& \textbf{80.6} & \textbf{80.6} & \textbf{5.8} & \textbf{+47.4}\\
\bottomrule
\multicolumn{5}{l}{\footnotesize $\dagger$ Structural SPS floor.
  Gain computed vs.\ bare MAC $<$50~ms column.}
\end{tabular}
\end{table}

\textbf{QDAP+DSRC} raises DENM deadline compliance from 53.8\% to 75.1\%
(+21.3~pp) by adding adaptive FEC that compensates for broadcast collisions
and priority scheduling that maps emergency messages to AC\_VO
(CW$_{\min}$=3), reducing effective collision exposure to $0.20\times$.

\textbf{QDAP+C-V2X} achieves the highest deadline compliance of any tested
stack (80.6\%).
For BSMs, standard SPS scheduling is retained.
For DENMs, QDAP triggers aperiodic one-shot transmission (3GPP~TS~36.321
\S{}5.8.1), bypassing the SPS resource pre-allocation window entirely.
This removes the structural $P(\text{miss}) = 0.5$ floor while FEC
compensates for the random collision exposure of aperiodic access.
The net result is +47.4~pp over bare C-V2X deadline compliance.

The implementation is unified: both adapters use the same
\texttt{qdap\_core} Rust library for FEC profile selection
(2.6~M decisions/s) and the same Python API.
A deployment engineer switches from DSRC to C-V2X by changing one
constructor argument; the application code and message formats
(SAE~J2735 BSM, ETSI~EN~302~637 DENM) remain identical.
The pure-Python fallback ensures compatibility on embedded ECUs
...
\subsection{Practical Safety Implications}

The 50~ms DENM deadline is not an arbitrary specification: it corresponds to
the minimum time for a human driver to initiate an avoidance response
($\approx$\SI{200}{\milli\second} reaction time) minus the worst-case
application processing and actuation latency
($\approx$\SI{150}{\milli\second}).
Failure to deliver within this window means the receiving vehicle's
safety application cannot trigger countermeasures before the hazard is
within unavoidable stopping distance.

At \SI{130}{\kilo\metre\per\hour} (\SI{36.1}{\metre\per\second}), each
\SI{10}{\milli\second} of additional latency translates to \SI{0.36}{\metre}
of additional blind travel distance.
A C-V2X DENM that misses the \SI{50}{\milli\second} deadline arrives, on
average, at $\approx\SI{75}{\milli\second}$ (midpoint of the SPS tail
$\sim\mathcal{U}[50, 100]$~ms), meaning the receiving vehicle travels an
extra \SI{2.7}{\metre} before the hazard alert is processed — on a highway
where the minimum safe stopping distance is \SI{73}{\metre}.
More critically, \textbf{67.5\%} of DENMs miss the deadline under C-V2X at
$N{=}75$: two in three emergency alerts arrive too late for compliant action.
MQTT is worse still: its 240~ms p99 latency corresponds to \SI{8.7}{\metre}
of blind travel at highway speed, exceeding the two-second following-distance
gap at \SI{60}{\kilo\metre\per\hour}.

QDAP's \SI{2.8}{\milli\second} emergency p99 — corresponding to
\SI{10}{\centi\metre} of vehicle travel — is effectively instantaneous at
all tested densities and speed profiles.
Equally important is the \emph{invariance} property: QDAP's p99 does not
degrade as vehicle density increases from $N{=}25$ to $N{=}100$, ensuring
that safety guarantees hold in both sparse rural and dense urban deployments.
This predictability is a prerequisite for safety certification under ISO~26262
and similar automotive safety integrity standards.
...
ETSI ITS-G5 or 3GPP PC5 radio access is future work, and the present
simulation assumes QDAP operates over the same channel as DSRC with a
priority-aware collision reduction model.
Multi-hop DENM relay (cascade propagation beyond one hop) is not yet
implemented.
The LOS/NLOS classification uses a \SI{350}{\metre} distance threshold
rather than a full ray-cast, which may overestimate LOS probability at
$N > 50$ with dense building occlusion.
MQTT is evaluated as a V2I broker-mediated protocol; its poor V2V deadline
performance reflects the inherent latency of two-hop (vehicle $\to$ broker
$\to$ vehicle) delivery and is not indicative of direct-link protocols.
...
Simulation across 20 Monte Carlo runs, five vehicle densities, and three V2X
scenarios demonstrates three headline results:
(i) QDAP delivers 74.0\% of DENMs within the \SI{50}{\milli\second} deadline
at $N{=}100$, versus 49.2\% for DSRC (+50\%, $p < 0.0001$) and 31.1\% for
C-V2X Mode~4 (2.4$\times$, $p{=}0.003$, Mann-Whitney U);
(ii) emergency p99 latency of \SI{2.8}{\milli\second} is invariant to vehicle
density, in contrast to MQTT (225--266~ms) and C-V2X (100~ms structural floor);
...
The 20-run ablation study quantifies that adaptive FEC contributes the PDR gain
($-6.2$~pp without it) while the scheduler contributes the latency guarantee
(3.4$\times$ reduction), confirming complementary and non-redundant roles.
